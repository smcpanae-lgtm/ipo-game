...
# ─────────────────────────────────────────────
# 売上成長イベントの報告文バリエーション
#   毎期発火するため、表示の都度ランダムに選択して単調さを防ぐ。
#   選択肢A/B/C（効果）は変えないので、どの文面でも成立する内容にする。
# ─────────────────────────────────────────────
SALES_GROWTH_DESCRIPTIONS = {
    "sales_growth_early": [
        # 1. 進捗未達（オリジナル）
        "営業部長からの報告：「今期の売上進捗は計画比80%です。\n"
        "残りの期間での打ち手について、社長のご判断をお願いします。\n"
        ...
        "【ポイント】IPO審査では継続的な売上成長と成長の再現性が重視されます。",
        # 2. 大型商談の好機
        "営業部長からの報告：「大型案件の商談が複数、最終局面に入っています。\n"
        "ここで営業体制を増強すれば受注確度を大きく上げられますが、\n"
        "投資負担も小さくありません。経営資源の配分をご判断ください。\n"
        "【ポイント】IPO審査では継続的な売上成長と成長の再現性が重視されます。",
        # 3. 解約率の上昇傾向
        "営業部長からの報告：「既存顧客の解約率がじわりと上昇しています。\n"
        "新規獲得で穴を埋めにいくか、既存顧客のフォローを固めるか——\n"
        "今期の営業方針について、社長のご判断をお願いします。\n"
        "【ポイント】IPO審査では継続的な売上成長と成長の再現性が重視されます。",
        # 4. 競合の攻勢
        "営業部長からの報告：「競合が大幅値引きでシェアを取りに来ています。\n"
        "正面から迎え撃つには相応の投資が必要です。\n"
        "どの方向性で経営資源を投入しますか？\n"
        "【ポイント】IPO審査では継続的な売上成長と成長の再現性が重視されます。",
    ],
    "sales_growth_late": [
        ...
        "【ポイント】申請期の業績は公募価格と時価総額に直接影響します。",
        # 2. 主幹事からの示唆
        "営業部長からの報告：「主幹事証券から『直前期・申請期の成長率は\n"
        "公募価格の算定に直結する』と改めて指摘を受けました。\n"
        "この時期の業績づくりについて、経営方針をご判断ください。\n"
        "【ポイント】申請期の業績は公募価格と時価総額に直接影響します。",
        # 3. 機関投資家の注目
        "営業部長からの報告：「上場が近づき、機関投資家がわが社の\n"
        "業績モメンタムに注目し始めています。ここで成長を示すか、\n"
        "収益性を固めるか——社長のご判断をお願いします。\n"
        "【ポイント】申請期の業績は公募価格と時価総額に直接影響します。",
        # 4. 期末の刈り取り判断
        "営業部長からの報告：「期末まで残りわずか。商談パイプラインには\n"
        "まだ刈り取れる案件が残っています。追加投資で積み上げるか、\n"
        "確実な着地を優先するか、ご判断ください。\n"
        "【ポイント】申請期の業績は公募価格と時価総額に直接影響します。",
    ],
}
...
EVENT_MARKET_SELECTION = GameEvent(
    ...
    description=(
        ...
        "【上場実務ポイント】市場区分の選択は資本政策・ガバナンス・IR体制を決める出発点です。"
    ),
    choices=[
        Choice(
...
EVENT_INSIDER_TRADING = GameEvent(
    ...
    description=(
        ...
        "【上場実務ポイント】インサイダー情報管理規程と役職員教育は上場審査でも確認項目です。"
    ),
    choices=[
        Choice(
...
EVENT_WHISTLEBLOWER = GameEvent(
    ...
    description=(
        ...
        "【上場実務ポイント】内部通報制度はコンプライアンス体制の要です。"
    ),
    choices=[
        Choice(
...
EVENT_ESG_DISCLOSURE = GameEvent(
    ...
    description=(
        ...
        "【上場実務ポイント】倫理・社会的責任は上場会社の持続可能性の要です。"
    ),
    choices=[
        Choice(
...
EVENT_STOCK_SPLIT = GameEvent(
    ...
    description=(
        ...
        "検討すべきタイミングです。」\n"
        "【上場実務ポイント】株式分割は流動性・株主数・公開価格の3要素を動かす重要な資本政策。"
    ),
    choices=[
        Choice(
...
EVENT_PREFERRED_CONVERSION = GameEvent(
    ...
    description=(
        ...
        "【上場実務ポイント】種類株式の上場時取扱いは資本政策の重要論点です。"
    ),
    choices=[
        Choice(
...
EVENT_DISCLOSURE_DRILL = GameEvent(
    ...
    description=(
        ...
        "【上場実務ポイント】適時開示は上場会社最重要のコンプライアンス義務の一つです。"
    ),
    choices=[
        Choice(
...
ALL_EVENTS = [
    ...
    EVENT_WHISTLEBLOWER,         # 【上場実務ポイント】内部通報制度（N-3〜N-2）
    ...
    EVENT_INSIDER_TRADING,       # 【上場実務ポイント】インサイダー取引防止
    EVENT_ESG_DISCLOSURE,        # 【上場実務ポイント】ESG/サステナビリティ開示
    EVENT_PREFERRED_CONVERSION,  # 【上場実務ポイント】種類株式の転換（資本政策）
    ...
    EVENT_STOCK_SPLIT,           # 【上場実務ポイント】株式分割（資本政策・N-1〜N）
    EVENT_DISCLOSURE_DRILL,      # 【上場実務ポイント】適時開示ドリル
    EVENT_LOCKUP_POLICY,         # 【上場実務ポイント】ロックアップ設定
